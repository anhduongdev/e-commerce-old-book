...
export function Footer() {
  return (
    <footer className="border-t border-brand-100 bg-cream-dark">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 md:grid-cols-5">
        <div className="md:col-span-2">
          <Logo />
          <p className="mt-4 max-w-xs text-sm text-brand-600">
          ...
          <div className="mt-4 flex gap-3">
            {SOCIAL_LINKS.map(({ label, initials }) => (
              <Link
                ...
                href="#"
                aria-label={label}
                className="flex h-9 w-9 items-center justify-center rounded-full bg-brand-700 text-xs font-semibold text-white transition-colors hover:bg-brand-900"
              >
                {initials}
              </Link>
            ))}
        ...
        <div>
          ...
          <h3 className="mt-6 text-sm font-semibold tracking-wide text-brand-900">
            ĐĂNG KÝ NHẬN TIN
          </h3>
          <form className="mt-3 flex overflow-hidden rounded-full border border-brand-100 bg-white">
            <input
              type="email"
            ...
            <button
              ...
              className="flex items-center bg-brand-700 px-4 text-white hover:bg-brand-900"
            >
      ...
      <div className="border-t border-brand-100">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 text-xs text-brand-600">
          <span>© 2024 Tiệm Sách Xưa. All rights reserved.</span>
          <div className="flex flex-wrap items-center gap-2">
            {PAYMENT_BADGES.map((badge) => (
              <span
                key={badge}
                className="rounded border border-brand-100 bg-white px-2 py-1 font-medium text-brand-700"
              >
                {badge}
              </span>
            ))}
            <span className="rounded border border-brand-100 bg-white px-2 py-1 font-medium text-brand-700">
